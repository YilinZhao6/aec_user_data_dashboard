...
function LineChart() {
  return (
    <svg className="sample5-line-chart" viewBox="0 0 720 250" preserveAspectRatio="none">
      {[50, 100, 150, 200].map((y) => (
        <line key={y} x1="0" x2="720" y1={y} y2={y} />
      ))}
      <path d="M0 174 C76 132 112 152 170 120 C224 90 276 102 334 82 C412 56 456 112 520 96 C594 78 632 54 720 70" className="sample5-chart-line main" />
      <path d="M0 206 C82 180 132 194 190 170 C244 148 304 174 366 138 C438 96 508 162 572 126 C640 88 678 124 720 112" className="sample5-chart-line muted" />
    </svg>
  )
}
...
function MetricGrid({ items }) {
  return (
    <section className="sample5-metrics">
      {items.map(([label, value, note]) => (
        <article key={label} className="sample5-metric">
          <span>{label}</span>
          <strong>{value}</strong>
          <p>{note}</p>
        </article>
...
function GeneralView() {
  return (
    <>
      <section className="sample5-intro">
      ...
      <section className="sample5-metrics">
        {overviewMetrics.map((metric) => (
          <article key={metric.label} className="sample5-metric">
      ...
      <section className="sample5-grid">
        <article className="sample5-panel sample5-wide">
          <div className="sample5-panel-heading">
            <div>
        ...
        <article className="sample5-panel">
          <div className="sample5-panel-heading">
            <div>
              <span>Activity</span>
...
function SampleTable() {
  return (
    <div className="sample5-table-wrap">
      <table className="sample5-table">
        <thead>
          <tr>
            <th>User</th>
            <th>Country</th>
...
function PlaceholderView({ tabId }) {
  ...
  return (
    <>
      <section className="sample5-intro compact">
...
export default function SampleDashboard5() {
  const [activeTab, setActiveTab] = useState('general')

  return (
    <div className="sample5-page">
      <nav className="sample5-tabs" aria-label="Dashboard sections">
        {tabs.map((tab) => (
          <button
            key={tab.id}
            type="button"
