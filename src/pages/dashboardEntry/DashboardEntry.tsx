...
export default function DashboardEntry() {
  ...
  const topUsers = useMemo(() => {
    ...
    const ranked: CountEntry[] = sortedEntries.map(([uid, value]) => ({
      name: friendly(uid),
      value,
    }));

    const rows: TopUserRow[] = sortedEntries.slice(0, topK).map(([uid, conversations]) => {
                  ...
                  <tbody>
                    {topUsers.rows.map((row) => {
                      ...
                      const top3 = Array.isArray(row.mostUsedFunctions)
                        ...
                        : [];
                      const sourceStr = row.acquisitionSources == null
                        ? '—'
                      ...
                      return (
                        <React.Fragment key={row.user_id}>
                          ...
                          >
                            ...
                            <td style={{ fontSize: 11, color: '#555', maxWidth: 120, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                                title={sourceStr !== '—' ? sourceStr : undefined}>
                              {sourceStr}
                            </td>
                          </tr>
